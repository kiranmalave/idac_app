...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	{
		$this->db->select("t.*,u.address,u.contactNo,u.whatsappNo,u.dateOfBirth,u.isVerified,u.isEmailSend",false);
		$this->db->from('admin as t');
		$this->db->where("t.adminID",$adminID);
		$this->db->join('user_extra_details as u', 'u.adminID = t.adminID');
		$query = $this->db->get();
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,dirname(__FILE__),__LINE__,__METHOD__);
		$result = $query->result();
	...
	{
		$this->db->where("adminID",$adminID);
		$res = $this->db->update("user_extra_details",$data);
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,dirname(__FILE__),__LINE__,__METHOD__);
		return $res;
	}
	...
	{
		$isinsert = $this->db->insert("user_extra_details",$data);
		$sqlerror = $this->db->error();
		$this->errorlogs->checkDBError($sqlerror,dirname(__FILE__),__LINE__,__METHOD__);
		return $isinsert;
	}
