	...
	public function userRoleChangeStatus()
	{
		...
		$action = $this->input->post("action");
			if(trim($action) == "changeStatus"){
				$ids = $this->input->post("list");
				$statusCode = $this->input->post("status");
				$changestatus = $this->CommonModel->changeMasterStatus('user_role_master',$statusCode,$ids,'roleID');
				// $idlist = explode(",",$ids);
				// $where = array("isDelete"=>"N","role !="=>"0");
				// $roleIDs=$roleList=$result=array();
				// $roleList = $this->CommonModel->getMasterDetails('user_role_master','roleID',$where);
				// $roleIDs = array_column($roleList, 'roleID');
				// $result=array_intersect($idlist,$roleIDs);
				// print_r($result);exit;
				// if(!empty($result)){
				//     $status['data'] = array();
				// 	$status['msg'] = "You Can't Delete Role Admin & POC";
				// 	$status['statusCode'] = 996;
				// 	$status['flag'] = 'F';
				// 	$this->response->output($status,200);
				// }else
