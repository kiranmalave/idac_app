define([
  ...
  var taxInvoiceSingleView = Backbone.View.extend({
    ...
    rowTotal: function (e) {
      ...
      $("tr.item-list-box").each(function (key, value) {
        ...
        if (rDisc != '' ) {
          ...
          if (rDiscType == 'amt') {
            console.log('rDiscType : ',rDiscType);
            rowtotal = rowtotal - (rDisc * parseFloat($("#itemQuantity_" + lastDetails).val()) )  ;
            console.log('rowtotal : ',rowtotal);
    ...
    setProduct: function (e) {
      ...
      $('#itemRate_'+lastDetails).val(price);
      $('#itemGST_'+lastDetails).val(gst);

      if (discount =='' || discount == 0) {
        $('#itemDiscount_'+lastDetails).val(0);
      }else
      { 
        $('#itemDiscount_'+lastDetails).val(discount);
      }

      $('#itemQuantity_'+lastDetails).val(1);
      if (with_gst == 'yes') {
        $('#itemAmtWithGST_'+lastDetails).prop('checked',true);
      }else
      {
